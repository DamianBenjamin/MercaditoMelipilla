...
import { ClipboardList, Plus, Trash2, AlertCircle, Info, CheckCircle2 } from 'lucide-react';
...
const NotasInventario = () => {
  ...
  const EstilosPrioridad = {
    rojo: "bg-red-50 border-red-200 text-red-700 shadow-sm",
    amarillo: "bg-amber-50 border-amber-200 text-amber-800 shadow-sm",
    verde: "bg-green-50 border-green-200 text-green-700 shadow-sm"
  };
  ...
  const agregarNota = async () => {
    ...
    try {
      const nueva = { texto, prioridad, fecha: new Date().toLocaleDateString(), resuelta: false };
  ...
  return (
    /* 🎨 MODIFICADO: Tarjeta blanca unificada con borde chocolate sutil */
    <div className="w-full bg-white border border-[#3D2517]/20 p-6 rounded-[2rem] shadow-xl shadow-[#3D2517]/5 animate-fade-in">
      <div className="flex items-center gap-2 mb-5">
        ...
        {/* Tipografía unificada a Sans idéntica a la de los formularios */}
        <h4 className="text-[#3D2517] font-sans font-black uppercase text-xs tracking-wider">
          Muro Compartido
        </h4>
      </div>

      <div className="space-y-4 mb-6 bg-slate-50 p-3 rounded-2xl border border-slate-200">
        <textarea 
          value={texto}
        ...
        <div className="flex flex-col gap-3">
          <div className="flex justify-center gap-4 bg-white py-2 rounded-xl border border-slate-200">
            {['rojo', 'amarillo', 'verde'].map((col) => (
              <button
                key={col}
                onClick={() => setPrioridad(col)}
                className={`w-5 h-5 rounded-full transition-transform cursor-pointer ${prioridad === col ? 'ring-2 ring-[#3D2517] scale-110' : 'opacity-40'} ${col === 'rojo' ? 'bg-red-500' : col === 'amarillo' ? 'bg-amber-500' : 'bg-green-500'}`}
              />
            ))}
          </div>
          {/* 🎨 BOTÓN CEREZA */}
          <button onClick={agregarNota} className="w-full py-2 bg-[#D91A3D] hover:bg-[#b81431] text-white text-xs font-black uppercase rounded-xl flex items-center justify-center gap-1 transition-all shadow-sm active:scale-95 cursor-pointer">
      ...
      <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
        {notas.map((nota) => (
          <div key={nota.id} className={`p-4 rounded-2xl border transition-all ${EstilosPrioridad[nota.prioridad] || 'bg-slate-50'}`}>
            <div className="flex justify-between items-start gap-2">
              ...
              <button onClick={() => eliminarNota(nota.id)} className="text-current opacity-30 hover:opacity-100 transition-opacity cursor-pointer">
                <Trash2 size={14} />
              </button>
            </div>
            <div className="flex justify-between items-center mt-3 pt-2 border-t border-black/5">
              ...
              <div className="flex gap-2 bg-white/60 p-1.5 rounded-xl border border-black/5 shadow-inner">
                {['rojo', 'amarillo', 'verde'].map((col) => (
                  <button key={col} onClick={() => cambiarColorNota(nota.id, col)} className={`w-4 h-4 rounded-full border border-black/10 transition-all hover:scale-125 cursor-pointer ${col === 'rojo' ? 'bg-red-500' : col === 'amarillo' ? 'bg-amber-500' : 'bg-green-500'} ${nota.prioridad === col ? 'ring-2 ring-white shadow-sm scale-110' : 'opacity-30'}`} />
                ))}
              </div>
            </div>
          </div>
